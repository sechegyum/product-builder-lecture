...
JOBS = [
    ("reto-plan.png",   1000, 290, "photo-company.jpg", (0, 230, 1620,  700)),  # 2번 · 생태 마스터플랜
    ("reto-office.jpg", 1000, 372, "photo-reason.jpg",  (0, 300, 2178, 1110)),  # 3번 · 본사 로비 사인
    ("reto-chart.jpg",  1000, 500, "photo-chart.jpg",   (0,  12, 1139,  890)),  # 4번 · 토스 일봉 캡처
    # 2번 원본은 1620x910 (비 1.78) 이라 가운데 띠만 남겨 비 3.45 로 맞췄다.
    #   청록 하천 · 도심 블록 · 라벨이 들어가고 우상단 RETO 워드마크는 잘린다 (카드에 로고가 이미 있다).
    # 3번 원본은 2178x1888 (비 1.15) 이라 초록 삼각 사인과 RETO 글자만 남겨 비 2.69 로 맞췄다.
    # 4번은 봉을 그리지 않는다 — 0.19~20.85 를 한 화면에 선형으로 그리면 13개 봉이 바닥에 깔려
    #   아무것도 안 읽힌다. 대표님이 주신 캡처가 마커까지 달고 있어 그대로 쓴다.
    #   원본 비 1.27 로 슬롯(2.0)보다 좁아 좌우에 블러 배경이 깔린다.
]
...
CHIP = {"src": "logo.png", "out": "logo-chip.png"}
